        ...
        <div className="p-4">
          ...
          <div className="grid grid-cols-2 gap-y-4 justify-items-center items-center">
            {data.map((record, index) => (
              <Link href={`/expenses?date=${new Date(record.date).getTime()}&remain=${record.remain || record.amount}`} key={record.date || index} className="relative px-3 py-2 border border-gray-700 rounded-xl w-[95%] overflow-hidden">
