    ...
    return (<>
        ...
        <div className="p-4">
            ...
            <div className="space-y-4">
                ...
                {
                    expens.map((item, idx) => (
                        <div key={idx} className="flex justify-between text-center items-center border-y-1 py-1.5 mx-0.5">
                            <p className={`text-green-400 w-[20%]`}>{Number(item.amount).toLocaleString('en-US')}</p>
                            <p className='text-amber-400 w-[55%] text-xs'>{item.reasion}</p>
                            <p className={`text-green-400 w-[20%]`}>{item.remain}</p>
                        </div>
                    ))
                }
            </div>
            <button onClick={() => setIsOpen(true)} className="absolute bottom-6 right-6 bg-[#2c2c4b] z-20 rounded-full"><Plus className='w-14 text-slate-100' /></button>
        </div>
    </>);
}